...
from releaseherald.version import VERSION
...
def get_version_news(pm, repo, commit_from, commit_to) -> VersionNews:
    news: List[News] = []
...
@click.option(
    ...
    help="Path to the config file, if not provided releaseherald.toml "
    "or pyproject.toml used from git repo root.",
)
@click.pass_context
def setup(
